CensusAPI2010s<-function(variables,state.fips,level=c("county","tract","block group","block","cdp","congressional_district"),key,summaryfile=c("sf1","ACS"))
{
  CensusData2010.sub<-function(variables,state.fips,level=c("county","tract","block group","block","cdp","congressional_district"),key,summaryfile=c("sf1","ACS"))
  {
    ...
    APIcall<-function(fipscode)
    {
      if(level=="county"&&summaryfile=="sf1")
        url<-paste("https://api.census.gov/data/2010/",summaryfile,"?get=",gsub(", ",",",toString(variables)),"&for=county:*&in=state:",state.fips,"&key=",key,sep="")
      
      else if(level=="county"&&summaryfile=="ACS")
        url<-paste("https://api.census.gov/data/2010/acs5?get=",gsub(", ",",",toString(variables)),"&for=county:*&in=state:",state.fips,"&key=",key,sep="")
      
      else if(level=="tract"&&summaryfile=="ACS")
        url<-paste("https://api.census.gov/data/2010/acs5?get=",gsub(", ",",",toString(variables)),"&for=tract:*&in=state:",state.fips,"&key=",key,sep="")
      
      else if(level=="block group"&&summaryfile=="ACS")
        url<-paste("https://api.census.gov/data/2010/acs5?get=",gsub(", ",",",toString(variables)),"&for=block+group:*&in=state:",state.fips,"+county:",fipscode,"&key=",key,sep="")
      
      else if(level=="congressional_district"&&summaryfile=="ACS")
        url<-paste("https://api.census.gov/data/2010/acs5?get=",gsub(", ",",",toString(variables)),"&for=congressional+district:*&in=state:",state.fips,"&key=",key,sep="")
      
      else if(level =="tract"&&summaryfile=="sf1")
        url<-paste("https://api.census.gov/data/2010/",summaryfile,"?get=",gsub(", ",",",toString(variables)),"&for=tract:*&in=state:",state.fips,"&key=",key,sep="")
      
      else if(level =="block group"&&summaryfile=="sf1")
        url<-paste("https://api.census.gov/data/2010/",summaryfile,"?get=",gsub(", ",",",toString(variables)),"&for=block+group:*&in=state:",state.fips,"+county:",fipscode,"&key=",key,sep="")
      
      else if(level =="block"&&summaryfile=="sf1")
        url<-paste("https://api.census.gov/data/2010/",summaryfile,"?get=",gsub(", ",",",toString(variables)),"&for=block:*&in=state:",state.fips,"+county:",substr(fipscode,1,3),"+tract:",substr(fipscode,4,9),"&key=",key,sep="")
      
      else if(level=="cdp"&&summaryfile=="sf1")
        url<-paste("https://api.census.gov/data/2010/",summaryfile,"?get=",gsub(", ",",",toString(variables)),"&for=place:*&in=state:",state.fips,"&key=",key,sep="")
      
      else if(level=="congressional_district"&&summaryfile=="sf1")
        url<-paste("https://api.census.gov/data/2010/",summaryfile,"?get=",gsub(", ",",",toString(variables)),"&for=congressional+district:*&in=state:",state.fips,"&key=",key,sep="")
      
      document <- try(rjson::fromJSON(file=url),silent=TRUE)
      if(class(document)=="try-error")
      {
        stop("Data could not be found.  Make sure to check https://api.census.gov/data/2010/sf1/variables.html to see if you have the right variable name")
      }
